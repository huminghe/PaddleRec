select sum(a.cnt), sum(b.cnt), count(a.cnt), count(b.cnt), count(d.cnt) from
...
and `date` >'2023-08-15' and `date` < '2023-10-15'
...
where a.cnt >= 5
and b.cnt is null and d.cnt is null
...
and `date` >'2023-07-04' and `date` < '2023-08-15'
group by distinct_id, author_id_str) a
left outer join
(select distinct_id, author_id_str, count(*) as cnt from events where
event = 'click' and click_element = 'CARD' and page_url = 'meet' and pay_user = 1
