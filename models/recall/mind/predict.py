...
author_country_map, _ = get_map(author_country_map_path, author_country_count)
...
ads_campaign_id_map, _ = get_map(ads_campaign_id_map_path, ads_campaign_count)
...
phone_height_id_map, _ = get_map(phone_height_id_map_path)
product_id_map, _ = get_map(product_id_map_path, product_count)
UNK_ID = 1
PADDING_ID = 0
...
def create_predict_data(author_list, history_country_list, country, ads_campaign, ads_group,
                        brand, phone_model, product):
    author_id_list = [author_id_map.get(x, UNK_ID) for x in author_list]
    author_country_list = [author_country_map.get(x.lower(), UNK_ID) for x in history_country_list]
    country_id = country_id_map.get(country.lower(), UNK_ID)
    ads_campaign_id = ads_campaign_id_map.get(ads_campaign, UNK_ID)
